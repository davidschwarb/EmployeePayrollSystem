...
public class PayrollSystem {
    ...
    public static void main(String[] args){
		...
		ps.initilizeEmployees();         //Initilizes each employee with the default constructor

		ps.menu();                                              //Calls the menu
    }
	public void initilizeEmployees(){
        for( int i = 0; i < employees.length; i++){
			
			employees[i] = new Employee();           //Initializes the each employee with the default constructor
        }
	}
	...
	public void saveEmployeesToFile(){
		...
		try (FileWriter fw = new FileWriter("PayrollReport.txt")){
			...
			fw.write("Employee            \t\tHourly  "
					+ "\t\tHours   \t\tGross   \t\tTaxed   \t\tNet \n"
					+ "Name                \t\tRate    \t\tWorked  \t\tAmount  \t\tAmount  \t\tAmount\n"
					+ "--------------------\t\t--------\t\t--------\t\t--------\t\t--------\t\t--------\n");
			...
			for(int i = 0; i < employees.length; i++)
			{
				...
				fieldLen = 8;
				fw.write(String.valueOf(f.format(employees[i].getPayRate())));//,employees[i].getHours());//,employees[i].getPayRate(),employees[i].getGrossPay(), employees[i].getNet());
				for(int j = 0;j < fieldLen - wordLen; j++)
					fw.write(" ");
				fw.write("\t\t");
